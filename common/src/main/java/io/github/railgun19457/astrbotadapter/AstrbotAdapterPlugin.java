...
public abstract class AstrbotAdapterPlugin {
    ...
    private void initializeServices() {
        ...
        // 绑定服务（QQ ↔ 游戏 ID → 白名单）
        BindingStore bindingStore = new BindingStore(
                dataFolder.toPath().resolve(config.getBindingStorageFile()), logger);
        bindingStore.load();
        bindingService = new BindingService(config, platformAdapter, bindingStore, logger);
        if (config.isBindingEnabled()) {
            logger.info("群友绑定功能已启用（白名单写入: "
                    + (config.isBindingApplyToWhitelist() ? "开" : "关") + "）");
        }

        // WebSocket入站消息处理
        if (unifiedServer != null) {
            // 绑定路由必须在 start() 之前注入
            unifiedServer.setBindingService(bindingService);
    ...
    private void handleWebSocketMessage(WebSocketSession session, Message message) {
        ...
        switch (message.getType()) {
            ...
            case BIND_REQUEST -> handleBindRequest(session, message);
            default -> {
                // ignore other types
            }
        }
     ...
     * WebSocket 绑定请求。
     *
     * <p>与 REST 的 {@code /api/v1/bindings} 复用同一个 {@link BindingService}，
     * 保证两条通道的行为、校验与白名单写入完全一致。</p>
     */
    private void handleBindRequest(WebSocketSession session, Message message) {
        if (unifiedServer == null) {
            return;
        }
        ...
        String action = JsonUtil.getString(payload, "action", "bind");
        String platform = JsonUtil.getString(payload, "platform", null);
        String userId = JsonUtil.getString(payload, "userId", null);

        if (bindingService == null) {
            sendCommandError(session, message, ErrorCode.FEATURE_DISABLED, "绑定功能不可用");
            return;
        }
        ...
        if ("unbind".equalsIgnoreCase(action)) {
            ...
            result = bindingService.bind(platform, userId, gameName, bedrock);
        }

        if (!result.isSuccess()) {
            ErrorCode errorCode = switch (result.getRejection()) {
            ...
            sendCommandError(session, message, errorCode, null);
            return;
        }
