...
public enum ErrorCode {
    ...
    // 绑定相关（4xxx）：与资源/参数类错误同段，HTTP 由 HttpRequestDispatcher 统一映射
    BINDING_NOT_FOUND(4004, "尚未绑定"),
    BINDING_NAME_TAKEN(4005, "该游戏 ID 已被其他账号绑定"),
    BINDING_INVALID_NAME(4006, "游戏 ID 格式不合法"),
    
    // 指令错误 (5xxx)
    COMMAND_EXECUTE_FAILED(5001, "指令执行失败"),
    COMMAND_FILTERED(5002, "指令被过滤"),
    COMMAND_NO_PERMISSION(5003, "无执行权限"),
    BINDING_WHITELIST_FAILED(5004, "白名单写入失败");
