...
public class ConfigValidator {
    ...
    public boolean validate(PluginConfig config) {
        ...
        // 验证绑定配置
        if (config.getBindingStorageFile() == null || config.getBindingStorageFile().isBlank()) {
            logger.warning("绑定数据文件名为空，已设为默认值 bindings.json");
            config.setBindingStorageFile("bindings.json");
            valid = false;
        }
        if (config.getGeyserPrefix() == null) {
            config.setGeyserPrefix(".");
            valid = false;
        }
