...
/**
 * NeoForge 平台 TPS/MSPT 统计器。
 *
 * <p>原版/NeoForge 服务端没有 Bukkit/Paper 的 {@code getTPS()} API，也没有 {@code /tps} 命令；
 * 这里通过订阅 {@link ServerTickEvent.Post}（每个 tick 结束、服务器完成本 tick 工作之后触发）
 * 自统计主线程 tick 间隔，按 1 分钟 / 5 分钟 / 15 分钟滑动窗口计算平均 TPS 与 MSPT。
 * 事件总线 API 在 26.2 稳定，无需任何前置 mod。</p>
 *
 * <p>线程安全：统计在主线程进行，结果写入 {@code volatile} 缓存；
 * REST（Netty 线程）读取时直接取缓存快照，不加锁。</p>
 */
public class NeoForgeTpsTracker {
