package io.github.railgun19457.astrbotadapter.platform.neoforge.command;
 ...
 * NeoForge 无权限插件，敏感子命令（reload / token / connections）要求 gamemaster 权限
 * （即原 OP 等级 2，见 {@link Commands#LEVEL_GAMEMASTERS}）。</p>
 */
public final class AstrbotCommand {
    ...
    private static int sendStatus(CommandContext<CommandSourceStack> ctx, AstrbotAdapterPlugin plugin) {
        ...
        if (adapter != null) {
            send(src, "&7平台: &f" + adapter.getServerName()
